# Prajwal Surya

Artificial Intelligence & Machine Learning Student

My goal is to contribute to the development of intelligent systems that create measurable impact across healthcare, agriculture, automation, and scientific research.

## Current Interests

- Machine Learning
- Large Language Models
- Computer Vision
- Data Analytics
- AI Evaluation
- Scientific Computing

Outside academics, I actively participate in hackathons, build AI projects, and continuously strengthen my practical AI and data science skills.

## 🛠️ Tech Stack

### Programming Languages
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?style=for-the-badge&logo=cplusplus&logoColor=white)

### Data Science & Machine Learning
![Pandas](https://img.shields.io/badge/Pandas-150458?style=for-the-badge&logo=pandas&logoColor=white)
![NumPy](https://img.shields.io/badge/NumPy-013243?style=for-the-badge&logo=numpy&logoColor=white)
![Matplotlib](https://img.shields.io/badge/Matplotlib-11557C?style=for-the-badge)
![Scikit-Learn](https://img.shields.io/badge/Scikit--Learn-F7931E?style=for-the-badge&logo=scikitlearn&logoColor=white)

### Development Tools
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)
![Jupyter](https://img.shields.io/badge/Jupyter-F37626?style=for-the-badge&logo=jupyter&logoColor=white)
![Google Colab](https://img.shields.io/badge/Google_Colab-F9AB00?style=for-the-badge&logo=googlecolab&logoColor=black)
![VS Code](https://img.shields.io/badge/VS_Code-007ACC?style=for-the-badge&logo=visualstudiocode&logoColor=white)

## Featured Projects

###  Mental Health AI Chatbot
- Developed an AI-powered mental health chatbot using a fine-tuned Large Language Model .
- Fine-tuned the Qwen model from Hugging Face using the LoRA (Low Rank Adaptation) technique .
- Designed the system to generate context-aware and empathetic conversational responses.

**Tech Stack:** Python • Hugging Face • Qwen • LoRA • LLM

---

###  AI Interior Design Assistant
- Built an AI-powered interior design assistant capable of generating design ideas and room layouts.
- Integrated Google's Gemini model to provide personalized design recommendations from user prompts.
- Focused on improving user interaction through generative AI.

**Tech Stack:** Python • Gemini API • Generative AI

---

###  Gardening Management System
- Designed and developed a gardening management system to organize plant information and gardening activities.
- Implemented features for tracking plant growth, maintenance schedules, and garden records.
- Built with scalability for future AI-assisted recommendations.

**Tech Stack:** Python

---

### 📊 Exploratory Data Analysis (Kaggle)

- Performed exploratory data analysis on real-world Kaggle datasets using Python.
- Cleaned missing values, analyzed feature distributions, and identified relationships between variables.
- Created visualizations to uncover patterns and support data-driven insights.
- Strengthened practical skills in data preprocessing and statistical analysis.

**Tech Stack:** Python • Pandas • NumPy • Matplotlib • Kaggle


## 🏆 Certifications & Achievements

- Kaggle Model Creator Certificate
- Microsoft AI Skills Fest Participation Badge
- Hackathon Participation Certificates (4)
- University Quiz Participation Certificate

## 📈 GitHub Statistics

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=Prajwalr3d&show_icons=true&theme=transparent" height="170"/>
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Prajwalr3d&layout=compact&theme=transparent" height="170"/>
</p>
